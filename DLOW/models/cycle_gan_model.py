...
class CycleGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        opt.use_sigmoid = opt.no_lsgan  # store_true, default false
        self.nlatent = opt.nlatent  # 16, z上采样后的维度
        self.label_intensity = opt.label_intensity  # 1, noise intensity
        ...
        self.netG_A = networks.define_stochastic_G(
            nlatent=opt.nlatent,  # 16
            input_nc=opt.input_nc,
            output_nc=opt.output_nc,
            ngf=64,
            which_model_netG=opt.which_model_netG,  # resnet_9blocks
            norm=opt.norm,  # instance
            use_dropout=opt.use_dropout,  # store_true，一般用于generator
            gpu_ids=opt.gpu_ids,
        )
        ...
        if self.isTrain:
            ...
            self.netD_A = networks.define_D_B(
                ...
                ndf=opt.ndf,  # 64
                which_model_netD=opt.which_model_netD,  # basic
                norm=opt.norm,  # instance
                use_sigmoid=opt.use_sigmoid,  # = opt.lsgan, store_true
                gpu_ids=opt.gpu_ids,
            )
        ...
        if self.isTrain:
            self.fake_A_pool = ImagePool(opt.pool_size)  # opt.pool_size:50
            self.fake_B_pool = ImagePool(opt.pool_size)
            # define loss functions
            self.criterionGAN = networks.GANLoss(
                use_lsgan=not opt.no_lsgan, tensor=self.Tensor  # opt.no_lsgan store ture, 所以not opt.no_lsgan = True
            )
        ...
        if not self.isTrain or opt.continue_train:
            self.load_networks(opt.which_epoch)  # opt.which_epoch = latest
        self.print_networks(opt.verbose)    # store true

    def set_input(self, input, beta, alpha, sign):
        AtoB = self.opt.which_direction == "AtoB"  # AtoB
        # input [1, 256, 256]
    ...
    def backward_D_A(self):  # 让net_D_A学会分辨真实的A，B。其中B为真，A为假。采用MSEloss
    ...
    def backward_D_B(self):  # 让net_D_B学会分辨真实的A，B。其中A为真，B为假，采用MSELoss
    ...
    def backward_D_A_classification(self, random_number):  # 让netD_A_classification学会分辨真假的B，采用Bceloss
        fake_B = self.fake_B_pool.query(self.fake_B)
    ...
    def backward_D_B_classification(self, random_number):  # 让netD_B_classification学会分辨真假的A，采用Bceloss
        fake_A = self.fake_A_pool.query(self.fake_A)
    ...
    def backward_D_A_classification_0(self, random_number):  # 让netD_A_classification_0学会分辨真A假B，采用Bceloss
        fake_B = self.fake_B_pool.query(self.fake_B)
    ...
    def backward_D_B_classification_0(self, random_number):  # 让netD_B_classification_0学会分辨真B假A，采用Bceloss
        fake_A = self.fake_A_pool.query(self.fake_A)
    ...
    def backward_G(self):
        ...
        lambda_A = self.opt.lambda_A  # 10, weight for cycle loss (A -> B -> A)
        lambda_B = self.opt.lambda_B  # 10
        lambda_GA = self.opt.lambda_GA  # 1
        lambda_GB = self.opt.lambda_GB  # 1
        lambda_GA_classification = self.opt.lambda_GA_classification  # 1
        lambda_GB_classification = self.opt.lambda_GB_classification  # 1
        lambda_GA_classification_0 = self.opt.lambda_GA_classification_0  # 1
        lambda_GB_classification_0 = self.opt.lambda_GB_classification_0  # 1
        # Identity loss
        if lambda_idt > 0:
    ...
    def optimize_parameters(self):
        # forward
        self.forward()  # forward()函数只是用于将input变成变量
        self.optimizer_DeConv.zero_grad()  # 清空过往梯度

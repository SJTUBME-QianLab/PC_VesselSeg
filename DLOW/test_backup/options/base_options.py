...
class BaseOptions():
    ...
    def initialize(self):
        self.parser.add_argument('--dataroot', type=str, default='/home/data/chenqz/MultiModaltoCT/',
                                 help='path to images (should have subfolders trainA, trainB, valA, valB, etc)')
        # 测试时调整这个目录，将\数据和预处理\multimodal_reprocess\images里面的所有图片从各自文件夹中提取出来放到一个大文件夹中，
        # 再将这个大文件夹的目录替换到上面的default中，这个目录在测试时候代表着储存测试数据的地址
        # self.parser.add_argument('--dataroot', required=True, help='path to images (should have subfolders trainA, trainB, valA, valB, etc)')
        self.parser.add_argument('--batchSize', type=int, default=1, help='input batch size')
        self.parser.add_argument('--loadSize', type=int, default=256, help='scale images to this size')#ori:286
        ...
        self.parser.add_argument('--name', type=str, default='MultiModaltoCT', help='name of the experiment. It decides where to store samples and models')
        ...
        self.parser.add_argument('--dataset_mode', type=str, default='single',
                                 help='chooses how datasets are loaded. [unaligned | aligned | single]') # test时修�?        # self.parser.add_argument('--model', type=str, default='cycle_gan',
        #                          help='chooses which model to use. cycle_gan, pix2pix, test')
        self.parser.add_argument('--model', type=str, default='test',
                                 help='chooses which model to use. cycle_gan, pix2pix, test') # test时修�?        self.parser.add_argument('--which_direction', type=str, default='AtoB', help='AtoB or BtoA')
        self.parser.add_argument('--nThreads', default=4, type=int, help='# threads for loading data')
        self.parser.add_argument('--checkpoints_dir', type=str, default='/home/data/chenqz/DLOW-Dirk-3/checkpoints', help='models are saved here')
